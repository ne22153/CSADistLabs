...
func medianWorker(startY, endY, startX, endX int, data func(y, x int) uint8, out chan<- [][]uint8, wg *sync.WaitGroup) {
	imageSlice := medianFilter(startY, endY, startX, endX, data)
	out <- imageSlice
	defer wg.Done()
}
...
func filter(filepathIn, filepathOut string, threads int) {
	...
	// immutableData is a closure, as makeImmutableMatrix returns a function
	...
	if threads == 1 {
		...
		for i := 0; i < threads; i++ {
			wg.Add(1)
			go medianWorker(i*(height/threads), (i+1)*(height/threads), 0, width, immutableData, genSlice[i], &wg)
		}
		wg.Wait()

		for _, element := range genSlice {
			for _, line := range <-element {
				newPixelData = append(newPixelData, line)
			}
		}
	}
